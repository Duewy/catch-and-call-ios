...
// =============================================================
// CatchEntryKgsView.swift
// Catch entry using Kg.hh (hundredths of kilograms)
// =============================================================

struct AddCatchKgsPopup: View {
    @Environment(\.dismiss) private var dismiss
    var speciesList: [String]
    var initialSpecies: String? = nil
    var initialKg: Int? = nil
    ...
    let onSave: (_ species: String, _ kg: Int, _ hundredths: Int) -> Void

    private var parsedKg:   Int { Int(kgText) ?? 0 }
    private var parsedHund: Int { Int(hundText) ?? 0 }

    private var canSave: Bool {
        let kOK = parsedKg >= 0
        let hOK = (0...99).contains(parsedHund)
        return kOK && hOK && (parsedKg * 100 + parsedHund) > 0
    }

    var body: some View {
        ZStack {
            Color.secondaryVariant.ignoresSafeArea()
            VStack(spacing: 16) {
                HStack { Text(title).font(.system(size: 24, weight: .bold)); Spacer(); Button("Cancel")
                    { dismiss() }.foregroundColor(.black) }
                ...
                VStack(alignment: .center, spacing: 6) {
                    Text("Weight").font(.system(size: 22, weight: .semibold)).foregroundColor(.black)
                    HStack(spacing: 6) {
                        Text("Kilograms").font(.system(size: 16, weight: .semibold)).foregroundColor(.black)
                        Text("Hundredths (00–99)").font(.system(size: 16, weight: .semibold)).foregroundColor(.black)
                    }.padding(.bottom, 4)

                    HStack(spacing: 6) {
                        Text(kgText.isEmpty ? "0" : kgText)
                            .font(.system(size: 30, weight: .semibold))
                            .foregroundStyle(.black)
                            .frame(width: 60, height: 40)
                            .background(padTarget == .pounds ? Color.white : Color.veryLiteGrey)
                            .clipShape(RoundedRectangle(cornerRadius: 10))
                            .overlay(RoundedRectangle(cornerRadius: 10).stroke(padTarget == .pounds ? Color.secondaryVariant : Color.veryLiteGrey, lineWidth: 2))
                            .onTapGesture { padTarget = .pounds; kgText = "" }
                        ...
                        Text(hundText.isEmpty ? "00" : hundText)
                            ...
                            .clipShape(RoundedRectangle(cornerRadius: 10))
                            .overlay(RoundedRectangle(cornerRadius: 10).stroke(padTarget == .ounces ? Color.secondaryVariant : Color.veryLiteGrey, lineWidth: 2))
                            .onTapGesture { padTarget = .ounces; hundText = "" }
                        Text("hh").foregroundColor(.black)
                    }

                    NumberPad(
                        text: padTarget == .pounds ? $kgText : $hundText,
                        target: padTarget,
        ...
        .onAppear {
            ...
            if let K = initialKg { kgText = String(K) }
...
// MARK: - Main View (Catch Entry – Kgs)

struct CatchEntryKgsView: View {
    ...
    // Only show catches that actually have Kg.hh stored
    private var kgsToday: [CatchItem] {
        vm.today.filter { ( $0.totalWeightHundredthKg ?? 0 ) > 0 &&
            ($0.catchType ?? "") == "Fun Day"
        }
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            if kgsToday.isEmpty {
                ...
                .alert(item: $confirmDelete) { item in
                    let totalHund = item.totalWeightHundredthKg ?? 0
                    let when = Date(timeIntervalSince1970: TimeInterval(item.dateTimeSec))
                    return Alert(
                        title: Text("Delete this catch?"),
                        message: Text("\(item.species.capitalized) – \(MeasureHelpers.formatKgsHundredth(totalHund)) at \(when.formatted(date: .omitted, time: .shortened))"),
                ...
                HStack {
                    Spacer()
                    if !kgsToday.isEmpty {
                        let totalCatches = kgsToday.count
                        let totHund = kgsToday.compactMap { $0.totalWeightHundredthKg }.reduce(0, +)

                        Text("Catches: \(totalCatches)").font(.subheadline).foregroundColor(.black)
                        Text("Total: \(MeasureHelpers.formatKgsHundredth(totHund))").font(.subheadline).foregroundColor(.black)
                    }
        ...
        .sheet(item: $editingItem) { item in
            let split: (Int, Int) = {
                if let t = item.totalWeightHundredthKg {
                    let a = abs(t)
                    return (a / 100, a % 100)
                } else {
                    return (0, 0)
            ...
            AddCatchKgsPopup(
                ...
                title: "Edit Catch"
            ) { species, wholeKg, hundredths in
                vm.updateCatchKgsHundredth(original: item, species: species, wholeKg: wholeKg, hundredths: hundredths)
            }
        }
        
        // -- ADD Catch Information PopUp ----
        .sheet(isPresented: $showingAddPopup) {
            AddCatchKgsPopup(speciesList: speciesList) { species, kgs, hundredths in
        ...
        .toolbarBackground(Color.secondaryVariant, for: .navigationBar)
...
private struct CatchRowKgs: View {
    let item: CatchItem
    var body: some View {
        HStack(spacing: 22) {
            Spacer()
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(item.species.capitalized)
                    .font(.headline)
                    .foregroundStyle(.black)
                if let hKg = item.totalWeightHundredthKg {
